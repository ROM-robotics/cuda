...
__global__ void sharedMemorySearch(int *input, int totalFound, int numElements, int numThreads)
{
    ...
    if (threadId < numThreads)
    {
        ...
        // Load this thread's span from global memory into shared memory
        for (int i = 0; i < span; i++)
            sharedInput[offset + i] = input[offset + i];
    }
    __syncthreads();
    ...
    if (threadId < numThreads)
    {
        ...
        {
            // Decrement the input array at index i by 1, update in memory prior to testing if it is equal to constant_search_value
            sharedInput[offset + i] -= 1;
            if (sharedInput[offset + i] == constant_search_value)
            {
                sharedInput[offset + i] = 1;
            }
            // Test if the new value is divisible by 2 (don't do anything with it trying to add some time for conditional branching)
            if (sharedInput[offset + i] % 2 == 0) {}
        }
    }
    __syncthreads();
    ...
    if (threadId < numThreads)
    {
        ...
        int offset = threadId * span;
        for (int i = 0; i < span; i++)
            input[offset + i] = sharedInput[offset + i];
    }
}
...
__global__ void constantMemorySearch(int *output)
{
    ...
    if (threadId < constant_num_threads)
    {
        ...
        {
            // Decrement the input array at index i by 1, update in memory prior to testing if it is equal to constant_search_value
            // constant_input is read-only; compute the decremented value in a local variable
            int val = constant_input[offset + i] - 1;
...
__global__ void registerMemorySearch(int *input, int totalFound, int numElements, int numThreads)
{
    ...
    if (threadId < numThreads)
    {
        ...
        int span = numElements / numThreads;
        int offset = threadId * span;
        // Use register variables so the compiler keeps these scalars on-chip
        register int val;
        for (int i = 0; i < span; i++)
        {
            // Decrement the input array at index i by 1, update in memory prior to testing if it is equal to constant_search_value
            val = input[offset + i] - 1;
            input[offset + i] = val;
            if (val == constant_search_value)
            {
                input[offset + i] = 1;
...
__host__ void executeKernel(int *d_input, int numElements, int threadsPerBlock, std::string kernelType)
{
    ...
    int span = numElements / threadsPerBlock;
    ...
    {
        constantMemorySearch<<<1,threadsPerBlock>>>(d_input);  // you will need to fill in function arguments appropriately
...
int main(int argc, char *argv[])
{
    ...
    // Cap total elements at MAX_ELEMENTS (16K) to stay within constant memory limit;
    // keep threadsPerBlock and reduce elementsPerThread accordingly
    if (numElements > MAX_ELEMENTS)
    {
        elementsPerThread = MAX_ELEMENTS / threadsPerBlock;
    ...
    outputfile << "partId: " << currentPartId.c_str() << " elements: " << elementsPerThread  << " threads: " << threadsPerBlock << " kernel: " << kernelType.c_str() << " elapsed time: " << (elapsedTime / 1000.0f) << "\n";
    outputfile.close();
